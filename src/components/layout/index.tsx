...
const Layout = ({ children }: React.PropsWithChildren<{}>) => {
  ...
  return (
    <>
      <header>
        <nav
          className={`fixed w-full z-200 px-4 laptop:px-16 duration-250 transition-all before:absolute before:block before:bg-red-700 before:h-full before:transition-all before:duration-250 before:left-0 before:w-full ${navBackground}`}
        >
          <div className="flex flex-row text-white items-center relative z-200">
            ...
            <ul className="hidden laptop:flex ml-auto mr-0 flex-row text-xs desktop:text-sm font-medium tracking-wide">
        ...
        <div
          className={`p-8 z-200 w-full h-screen laptop:hidden absolute bg-red-700 transition-all duration-700 ${mobileMenuOpen ? 'translate-x-0 overscroll-contain' : ' -translate-x-full'}`}
        >
          <div className="w-full h-4 flex flex-row justify-end">
            <button
              onClick={() => {
                toggleMobileMenu()
              }}
